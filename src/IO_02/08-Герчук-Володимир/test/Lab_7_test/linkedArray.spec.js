...
describe('Lab-7: LinkedArray module', () => {
  ...
  describe('elementAt method', () => {
    ...
    it('should throw an error with index >= length', () => {
      expect(() => (new LinkedArray()).elementAt(100)).toThrow(Error('Invalid index (100)'));
      expect(() => (this.defaultLnk.elementAt(this.prevLength))).toThrow(Error(`Invalid index (${this.prevLength})`));
    });

    it('should throw an error with negative index', () => {
      expect(() => this.defaultLnk.elementAt(-4)).toThrow(Error('Invalid index (-4)'));
    });
  });
  ...
  describe('addAt method', () => {
    ...
    it('should throw an error with negative index', () => {
      expect(() => this.defaultLnk.addAt(-11, '🥔')).toThrow(Error('Invalid index (-11)'));
    });
  });
  ...
  describe('removeAt method', () => {
    ...
    it('should throw an error when index < 0', () => {
      expect(() => this.defaultLnk.removeAt(-1)).toThrow(Error('Invalid index (-1)'));
    });
    
    it('should throw an error when index >= length', () => {
      expect(() => this.defaultLnk.removeAt(this.prevLength)).toThrow(Error(`Invalid index (${this.prevLength})`));
    });
  });
});
